...
public interface IMovieRepository extends JpaRepository<MovieEntity, Integer> {
    ...
    List<MovieEntity> findMoviesWithActorRatingGreatingThan(@Param("rating") BigDecimal rating);


    @Query("SELECT g.movies FROM GenreEntity g WHERE g.name= :genre")
    List<MovieEntity> findMoviesInGenre(@Param("genre") String genre);
